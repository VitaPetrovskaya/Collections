...
public class Order : KeyedCollection<int, OrderItem>
{
    ...
    public OrderItem Find(int partNumber)
    {
        return OrderItems.Find(x => GetKeyForItem(x) == partNumber);
    }
}
...
public class Program
{
    public static void Main()
    {
        ...
        // TODO Replace with code that searches a list item in order.OrderItems. Use KeyedCollection<TKey, TItem> methods only.
        if (order.Find(111033401) == null)
        {
        ...
        // TODO Replace with code that searches a list item in order.OrderItems. Use KeyedCollection<TKey, TItem> methods only.
        if (order.Find(127700026) != null)
        {
            // TODO Calculate an order item price, and write it to the console using the same format as in Display method.
            Console.WriteLine("Order #2 has #127700026 item - price is {0}$.", order.Find(127700026).UnitPrice);
        }
